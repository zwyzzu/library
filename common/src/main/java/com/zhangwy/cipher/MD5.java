...
public class MD5 {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            try {
                ...
                while ((len = in.read(buffer, 0, 1024)) != -1) {
                    md5Impl.md5Update(buffer, len);
                }
                return md5Impl.hexString();
            } catch (IOException e) {
                Logger.e("md5EncodeFile", e);
            } finally {
                try {
                    if (in != null) {
                        in.close();
    ...
    private static class MD5Impl {
        ...
        MD5Impl(String src) {
            this((src == null || src.equals("")) ? "null".getBytes() : src.getBytes());
        }
        ...
        /**
         * md5Update是MD5的主计算过程，input是要变换的字节串，inputLen是长度
         **/
        void md5Update(byte[] input, int inputLen) {
            int i, index, partLen;
